...
def get_random(df):
    sample = random.sample(range(0, df.shape[0]-1), 5)
    df = df.iloc[sample, :]
    return df
...
def animation(df, outfile):
    ...
    def animate(i):
        ...
        ax.set_title("Population of 5 random countries, year "+str(1960+i), fontsize=18, fontweight='bold')
...
df_random= get_random(filtered_df)
animation(df_random, "../plots/pie_random.gif")
